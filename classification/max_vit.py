...
class RelativeSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        self.relative_position_bias_table = nn.Parameter(
            torch.zeros((2 * grid_window_size[0] - 1) * (2 * grid_window_size[1] - 1), self.num_heads))
...
class MaxViT(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            output (torch.Tensor): Classification output of the shape [B, num_classes].
        """
        output = self.forward_features(self.stem(x))
...
if __name__ == '__main__':
    ...
    def test_networks() -> None:
        ...
        for get_network in [max_vit_tiny_224]:
